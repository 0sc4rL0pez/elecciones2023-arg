...
if visualizacion==etapas[1]:
    ...
    if unidades1=='Porcentaje':
        df_cant_publi['cantidad'] = df_cant_publi[df_cant_publi['cantidad_likes']].map(lambda x: round(100*float(x)/sum_cant_pub,1))
    elif unidades1 == 'miles':
        df_cant_publi['cantidad'] = df_cant_publi[df_cant_publi['cantidad_likes']].map(lambda x: round(float(x)/1000,1))
